...
async function renderAsync(project: Project, skipOpen: boolean) {
  ...
  await exportPng(canvas, skipOpen);
}

export function render(project: Project, skipOpen = false) {
  renderAsync(project, skipOpen)
